...
public class Basic extends JFrame implements Runnable, KeyListener,
                                             ...
                                             WindowListener
{
  protected static int FPS = 60; // desired frames per second
  ...
  private int whichPad;  // maintain which pad's pixel grid region mouse is in

  private Graphics dbg;  
  private Image dbImage = null;

  protected ArrayList<Sketchpad> pads;
...
//    requestFocus();
  ...
  private void render()
  {
    ...
    {// create the buffer
      ...
      {// dbg is created, tell Sketchpad
        dbg = dbImage.getGraphics();
        Sketchpad.setGraphicsContext( dbg );
      }
    }
  ...
  private void paintScreen()
  {
    ...
    try{
      ...
      //   Toolkit.getDefaultToolkit().sync();  // hope not needed?
      gr.dispose();
    }
    catch(Exception e)
    {
  ...
  private void updateMouse( MouseEvent e )
  {
    ...
    // determine which pad this mouse event hits
    // and compute mouseX, mouseY in that pad's graph paper coords
    
    whichPad = -1;
    Sketchpad ac=null, temp;
    ...
    {// mouse event happened in an active pad, update coords for that pad
